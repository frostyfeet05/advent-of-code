...
const getMaxHeight = (startVelocity: Point, area: Area): number => {
    ...
    while (inArea !== PointInArea.AFTER) {
        ...
        if (inArea !== PointInArea.AFTER) {
            points.push({x: position.x, y: position.y, where: inArea});
        }
    }

    const maxHeight = hasHitInside ? points.map(p => p.y).reduce((a, b) => Math.max(a, b)) : -1;
    console.log(`Max height for velocity=${startVelocity.x},${startVelocity.y} is ${maxHeight === -1 ? 'INVALID' : maxHeight}.`)
    return maxHeight;
};
...
const solve = (input: string): number => {
    ...
    const list = Array.from({length: Math.max(area.x2, area.y2)}, (k,i) => i);
    list.forEach(y => list.forEach(x => maxHeights.push(getMaxHeight({x,y}, area))));

    return maxHeights.reduce((a, b) => Math.max(a, b));
};
